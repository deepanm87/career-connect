...
export const updateProfile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.targetRole !== undefined) {
      patch.targetRole = args.targetRole
    }
    if (args.openToWork !== undefined) {
      patch.openToWork = args.openToWork
    }
    ...
    if (updated === null) {
      throw new Error("Profile vanished during update")
    }
    return updated
  }
})
