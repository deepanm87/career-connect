...
async function embedTexts(inputs: string[]): Promise<number[][]> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`AI Gateway embeddings request failed (${res.status}): ${body}`)
  }
  ...
  if (!data || data.length !== inputs.length) {
    throw new Error(`AI Gateway returned ${data?.length ?? 0} embeddings for ${inputs.length} inputs`)
  }
